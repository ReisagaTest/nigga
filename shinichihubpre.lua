...
local scriptName = "ShinichiHub" 
local apiUrl = "https://shinichihub.vercel.app/api/server/verify-key.php" 
local debugMode = true
local validKeys = {"TaoFanShinicuHub"} 
local wlKey = "f8876aecc19f4d38db6dc5a021adff4b" 
...
local function getStoredOwner()
    local existingValue = workspace:FindFirstChild(scriptName)
    if existingValue and existingValue:IsA("StringValue") then
        return existingValue.Value
    end
    return nil
end

local function setStoredOwner(playerName)
    local ownerValue = workspace:FindFirstChild(scriptName)
    if not ownerValue then
        ownerValue = Instance.new("StringValue")
        ownerValue.Name = scriptName
    end
    ownerValue.Value = playerName
    ownerValue.Parent = workspace
end

local function checkScriptOwner(player)
    local currentOwner = getStoredOwner()
    if currentOwner then
        if currentOwner ~= player.Name then
            if not debugMode then
                player:Kick("❌ Script locked by another account: " .. currentOwner)
            else
                warn("❌ Script locked by: " .. currentOwner)
            end
            return false
        else
            print("✅ " .. player.Name .. " is the valid owner.")
            return true
        end
    else
        player:Kick("❌ Script locked by another account: " .. currentOwner)
...
local function verifyKey(player, inputKey)
    ...
    if not isWhitelisted then
        if not debugMode then
            player:Kick("❌ Not a valid key!")
        else
            warn("❌ Invalid key provided: " .. inputKey)
        end
        return false
    end
    ...
    if not success then
        if not debugMode then
            player:Kick("❌ Failed to contact verification server!")
        else
            warn("❌ Server request failed: " .. response)
        end
        return false
    end
    ...
    if not responseData.success then
        if not debugMode then
            player:Kick("❌ Key verification failed: " .. (responseData.message or "Unknown error"))
        else
            warn("❌ Verification failed: " .. (responseData.message or "Unknown error"))
        end
        return false
    end

    local storedKey = TeleportService:GetTeleportSetting("VerifiedKey")
    if storedKey and storedKey ~= inputKey then
        if not debugMode then
            player:Kick("❌ Do not try to spoof your key!")
        else
            warn("❌ Key spoofing detected!")
        end
        return false
    end
    TeleportService:SetTeleportSetting("VerifiedKey", inputKey)
...
local function executeKeySystem()
    ...
    if not checkScriptOwner(player) then
        return
    end

    local authStart = tick()
    if not verifyKey(player, inputKey) then
        return
    end

    print("✅ Key verified successfully!")
    notify("Whitelist Successfully!", 5)

    local success, err = pcall(function()
        local scriptUrl = "uinotifyvaoday"
        local loadedScript = loadstring(game:HttpGet(scriptUrl))()
        loadedScript.New("Script Loaded!", 5)
    end)

    if not success then
        warn("❌ Failed to load script: " .. err)
    else
        print("✅ Script loaded in " .. string.format("%.2f", tick() - authStart) .. " seconds!")
    end

    spawn(function()
        while wait(1) do
            if getgenv().Key ~= inputKey then
                if not debugMode then
                    player:Kick("❌ Key spoofing detected!")
                else
                    warn("❌ Key spoofing detected!")
                end
                break
...
local success, err = pcall(executeKeySystem)
if not success then
    player:Kick("❌ Key Not Vaild")
else
    repeat
        pcall(function()
    ...
    local HttpService = game:GetService("HttpService")
    local url = "https://shinichihub.vercel.app/api/status/bossapi"
    ...
    local Tabs = {
        ...
        PremiumOnly = Window:AddTab({
            Title = "Tab Premium Only",
            Icon = "gem"
        }),
        Main = Window:AddTab({
            Title = "Tab Farming",
    ...
    if (GamePlaceID == 2753915549) then
        Sea1 = true;
    elseif (GamePlaceID == 4442272183) then
        Sea2 = true;
    elseif (GamePlaceID == 7449423635) then
        Sea3 = true;
    else
        game:Shutdown();
    end
    ...
    function CheckLevel()
        ...
        if Sea1 then
            if ((Level == 1) or (Level <= 9) or (SelectMonster == "Bandit")) then
                Ms = "Bandit";
            ...
            elseif ((Level == 10) or (Level <= 14) or (SelectMonster == "Monkey")) then
                Ms = "Monkey";
            ...
            elseif ((Level == 15) or (Level <= 29) or (SelectMonster == "Gorilla")) then
                Ms = "Gorilla";
                NameQuest = "JungleQuest";
                QuestLv = 2;
                NameMon = "Gorilla";
            ...
            elseif ((Level == 30) or (Level <= 39) or (SelectMonster == "Pirate")) then
                Ms = "Pirate";
            ...
            elseif ((Level == 40) or (Level <= 59) or (SelectMonster == "Brute")) then
                Ms = "Brute";
                NameQuest = "BuggyQuest1";
                QuestLv = 2;
                NameMon = "Brute";
            ...
            elseif ((Level == 60) or (Level <= 74) or (SelectMonster == "Desert Bandit")) then
                Ms = "Desert Bandit";
            ...
            elseif ((Level == 75) or (Level <= 89) or (SelectMonster == "Desert Officer")) then
                Ms = "Desert Officer";
                NameQuest = "DesertQuest";
                QuestLv = 2;
            ...
            elseif ((Level == 90) or (Level <= 99) or (SelectMonster == "Snow Bandit")) then
                Ms = "Snow Bandit";
                NameQuest = "SnowQuest";
                QuestLv = 1;
                NameMon = "Snow Bandit";
            ...
            elseif ((Level == 100) or (Level <= 119) or (SelectMonster == "Snowman")) then
                Ms = "Snowman";
                NameQuest = "SnowQuest";
                QuestLv = 2;
                NameMon = "Snowman";
            ...
            elseif ((Level == 120) or (Level <= 149) or (SelectMonster == "Chief Petty Officer")) then
            ...
            elseif ((Level == 150) or (Level <= 174) or (SelectMonster == "Sky Bandit")) then
                Ms = "Sky Bandit";
                NameQuest = "SkyQuest";
                QuestLv = 1;
                NameMon = "Sky Bandit";
            ...
            elseif ((Level == 175) or (Level <= 189) or (SelectMonster == "Dark Master")) then
                Ms = "Dark Master";
                NameQuest = "SkyQuest";
                QuestLv = 2;
                NameMon = "Dark Master";
            ...
            elseif ((Level == 190) or (Level <= 209) or (SelectMonster == "Prisoner")) then
                Ms = "Prisoner";
            ...
            elseif ((Level == 210) or (Level <= 249) or (SelectMonster == "Dangerous Prisoner")) then
            ...
            elseif ((Level == 250) or (Level <= 274) or (SelectMonster == "Toga Warrior")) then
                Ms = "Toga Warrior";
            ...
            elseif ((Level == 275) or (Level <= 299) or (SelectMonster == "Gladiator")) then
                Ms = "Gladiator";
                NameQuest = "ColosseumQuest";
                QuestLv = 2;
                NameMon = "Gladiator";
            ...
            elseif ((Level == 300) or (Level <= 324) or (SelectMonster == "Military Soldier")) then
                Ms = "Military Soldier";
            ...
            elseif ((Level == 325) or (Level <= 374) or (SelectMonster == "Military Spy")) then
                Ms = "Military Spy";
                NameQuest = "MagmaQuest";
                QuestLv = 2;
            ...
            elseif ((Level == 375) or (Level <= 399) or (SelectMonster == "Fishman Warrior")) then
                Ms = "Fishman Warrior";
            ...
            elseif ((Level == 400) or (Level <= 449) or (SelectMonster == "Fishman Commando")) then
                Ms = "Fishman Commando";
                NameQuest = "FishmanQuest";
                QuestLv = 2;
            ...
            elseif ((Level == 10) or (Level <= 474) or (SelectMonster == "God's Guard")) then
                Ms = "God's Guard";
            ...
            elseif ((Level == 475) or (Level <= 524) or (SelectMonster == "Shanda")) then
                Ms = "Shanda";
                NameQuest = "SkyExp1Quest";
                QuestLv = 2;
                NameMon = "Shanda";
            ...
            elseif ((Level == 525) or (Level <= 549) or (SelectMonster == "Royal Squad")) then
                Ms = "Royal Squad";
            ...
            elseif ((Level == 550) or (Level <= 624) or (SelectMonster == "Royal Soldier")) then
                Ms = "Royal Soldier";
                NameQuest = "SkyExp2Quest";
                QuestLv = 2;
            ...
            elseif ((Level == 625) or (Level <= 649) or (SelectMonster == "Galley Pirate")) then
                Ms = "Galley Pirate";
        ...
        if Sea2 then
            if ((Level == 700) or (Level <= 724) or (SelectMonster == "Raider")) then
                Ms = "Raider";
            ...
            elseif ((Level == 725) or (Level <= 774) or (SelectMonster == "Mercenary")) then
                Ms = "Mercenary";
                NameQuest = "Area1Quest";
                QuestLv = 2;
                NameMon = "Mercenary";
            ...
            elseif ((Level == 775) or (Level <= 799) or (SelectMonster == "Swan Pirate")) then
                Ms = "Swan Pirate";
            ...
            elseif ((Level == 800) or (Level <= 874) or (SelectMonster == "Factory Staff")) then
                Ms = "Factory Staff";
                NameQuest = "Area2Quest";
                QuestLv = 2;
            ...
            elseif ((Level == 875) or (Level <= 899) or (SelectMonster == "Marine Lieutenan")) then
            ...
            elseif ((Level == 900) or (Level <= 949) or (SelectMonster == "Marine Captain")) then
                Ms = "Marine Captain";
                NameQuest = "MarineQuest3";
                QuestLv = 2;
            ...
            elseif ((Level == 950) or (Level <= 974) or (SelectMonster == "Zombie")) then
                Ms = "Zombie";
            ...
            elseif ((Level == 975) or (Level <= 999) or (SelectMonster == "Vampire")) then
                Ms = "Vampire";
                NameQuest = "ZombieQuest";
                QuestLv = 2;
                NameMon = "Vampire";
            ...
            elseif ((Level == 1000) or (Level <= 1049) or (SelectMonster == "Snow Trooper")) then
                Ms = "Snow Trooper";
            ...
            elseif ((Level == 1050) or (Level <= 1099) or (SelectMonster == "Winter Warrior")) then
                Ms = "Winter Warrior";
                NameQuest = "SnowMountainQuest";
                QuestLv = 2;
            ...
            elseif ((Level == 1100) or (Level <= 1124) or (SelectMonster == "Lab Subordinate")) then
                Ms = "Lab Subordinate";
            ...
            elseif ((Level == 1125) or (Level <= 1174) or (SelectMonster == "Horned Warrior")) then
                Ms = "Horned Warrior";
                NameQuest = "IceSideQuest";
                QuestLv = 2;
            ...
            elseif ((Level == 1175) or (Level <= 1199) or (SelectMonster == "Magma Ninja")) then
                Ms = "Magma Ninja";
            ...
            elseif ((Level == 1200) or (Level <= 1249) or (SelectMonster == "Lava Pirate")) then
                Ms = "Lava Pirate";
                NameQuest = "FireSideQuest";
                QuestLv = 2;
                NameMon = "Lava Pirate";
            ...
            elseif ((Level == 1250) or (Level <= 1274) or (SelectMonster == "Ship Deckhand")) then
                Ms = "Ship Deckhand";
            ...
            elseif ((Level == 1275) or (Level <= 1299) or (SelectMonster == "Ship Engineer")) then
                Ms = "Ship Engineer";
                NameQuest = "ShipQuest1";
                QuestLv = 2;
            ...
            elseif ((Level == 1300) or (Level <= 1324) or (SelectMonster == "Ship Steward")) then
                Ms = "Ship Steward";
            ...
            elseif ((Level == 1325) or (Level <= 1349) or (SelectMonster == "Ship Officer")) then
                Ms = "Ship Officer";
                NameQuest = "ShipQuest2";
                QuestLv = 2;
            ...
            elseif ((Level == 1350) or (Level <= 1374) or (SelectMonster == "Arctic Warrior")) then
                Ms = "Arctic Warrior";
            ...
            elseif ((Level == 1375) or (Level <= 1424) or (SelectMonster == "Snow Lurker")) then
                Ms = "Snow Lurker";
                NameQuest = "FrostQuest";
                QuestLv = 2;
                NameMon = "Snow Lurker";
            ...
            elseif ((Level == 1425) or (Level <= 1449) or (SelectMonster == "Sea Soldier")) then
                Ms = "Sea Soldier";
        ...
        if Sea3 then
            if ((Level == 1500) or (Level <= 1524) or (SelectMonster == "Pirate Millionaire")) then
            ...
            elseif ((Level == 1525) or (Level <= 1574) or (SelectMonster == "Pistol Billionaire")) then
            ...
            elseif ((Level == 1575) or (Level <= 1599) or (SelectMonster == "Dragon Crew Warrior")) then
            ...
            elseif ((Level == 1600) or (Level <= 1624) or (SelectMonster == "Dragon Crew Archer")) then
            ...
            elseif ((Level == 1625) or (Level <= 1649) or (SelectMonster == "Hydra Enforcer")) then
                Ms = "Hydra Enforcer";
            ...
            elseif ((Level == 1650) or (Level <= 1699) or (SelectMonster == "Venomous Assailant")) then
            ...
            elseif ((Level == 1700) or (Level <= 1724) or (SelectMonster == "Marine Commodore")) then
                Ms = "Marine Commodore";
            ...
            elseif ((Level == 1725) or (Level <= 1774) or (SelectMonster == "Marine Rear Admiral")) then
            ...
            elseif ((Level == 1775) or (Level <= 1799) or (SelectMonster == "Fishman Raider")) then
                Ms = "Fishman Raider";
            ...
            elseif ((Level == 1800) or (Level <= 1824) or (SelectMonster == "Fishman Captain")) then
                Ms = "Fishman Captain";
                NameQuest = "DeepForestIsland3";
                QuestLv = 2;
            ...
            elseif ((Level == 1825) or (Level <= 1849) or (SelectMonster == "Forest Pirate")) then
                Ms = "Forest Pirate";
            ...
            elseif ((Level == 1850) or (Level <= 1899) or (SelectMonster == "Mythological Pirate")) then
            ...
            elseif ((Level == 1900) or (Level <= 1924) or (SelectMonster == "Jungle Pirate")) then
                Ms = "Jungle Pirate";
            ...
            elseif ((Level == 1925) or (Level <= 1974) or (SelectMonster == "Musketeer Pirate")) then
                Ms = "Musketeer Pirate";
                NameQuest = "DeepForestIsland2";
                QuestLv = 2;
            ...
            elseif ((Level == 1975) or (Level <= 1999) or (SelectMonster == "Reborn Skeleton")) then
                Ms = "Reborn Skeleton";
            ...
            elseif ((Level == 2000) or (Level <= 2024) or (SelectMonster == "Living Zombie")) then
                Ms = "Living Zombie";
                NameQuest = "HauntedQuest1";
                QuestLv = 2;
            ...
            elseif ((Level == 2025) or (Level <= 2049) or (SelectMonster == "Demonic Soul")) then
                Ms = "Demonic Soul";
            ...
            elseif ((Level == 2050) or (Level <= 2074) or (SelectMonster == "Posessed Mummy")) then
                Ms = "Posessed Mummy";
                NameQuest = "HauntedQuest2";
                QuestLv = 2;
            ...
            elseif ((Level == 2075) or (Level <= 2099) or (SelectMonster == "Peanut Scout")) then
                Ms = "Peanut Scout";
            ...
            elseif ((Level == 2100) or (Level <= 2124) or (SelectMonster == "Peanut President")) then
                Ms = "Peanut President";
                NameQuest = "NutsIslandQuest";
                QuestLv = 2;
                ...
                CFrameQ = CFrame.new(- 2105.53198, 37.2495995, - 10195.5088, - 0.766061664, 0, - 0.642767608, 0, 1, 0, 0.642767608, 0, - 0.766061664);
                CFrameMon = CFrame.new(- 2150.587890625, 122.49767303467, - 10358.994140625);
            elseif ((Level == 2125) or (Level <= 2149) or (SelectMonster == "Ice Cream Chef")) then
                Ms = "Ice Cream Chef";
            ...
            elseif ((Level == 2150) or (Level <= 2199) or (SelectMonster == "Ice Cream Commander")) then
                ...
                CFrameQ = CFrame.new(- 819.376709, 64.9259796, - 10967.2832, - 0.766061664, 0, 0.642767608, 0, 1, 0, - 0.642767608, 0, - 0.766061664);
                CFrameMon = CFrame.new(- 789.941528, 209.382889, - 11009.9805, - 0.0703101531, "-0", - 0.997525156, "-0", 1.00000012, "-0", 0.997525275, 0, - 0.0703101456);
            elseif ((Level == 2200) or (Level <= 2224) or (SelectMonster == "Cookie Crafter")) then
                Ms = "Cookie Crafter";
            ...
            elseif ((Level == 2225) or (Level <= 2249) or (SelectMonster == "Cake Guard")) then
                Ms = "Cake Guard";
                NameQuest = "CakeQuest1";
                QuestLv = 2;
                NameMon = "Cake Guard";
            ...
            elseif ((Level == 2250) or (Level <= 2274) or (SelectMonster == "Baking Staff")) then
                Ms = "Baking Staff";
            ...
            elseif ((Level == 2275) or (Level <= 2299) or (SelectMonster == "Head Baker")) then
                Ms = "Head Baker";
                NameQuest = "CakeQuest2";
                QuestLv = 2;
                NameMon = "Head Baker";
            ...
            elseif ((Level == 2300) or (Level <= 2324) or (SelectMonster == "Cocoa Warrior")) then
                Ms = "Cocoa Warrior";
            ...
            elseif ((Level == 2325) or (Level <= 2349) or (SelectMonster == "Chocolate Bar Battler")) then
            ...
            elseif ((Level == 2350) or (Level <= 2374) or (SelectMonster == "Sweet Thief")) then
                Ms = "Sweet Thief";
            ...
            elseif ((Level == 2375) or (Level <= 2400) or (SelectMonster == "Candy Rebel")) then
                Ms = "Candy Rebel";
                NameQuest = "ChocQuest2";
                QuestLv = 2;
                NameMon = "Candy Rebel";
            ...
            elseif ((Level == 2400) or (Level <= 2424) or (SelectMonster == "Candy Pirate")) then
                Ms = "Candy Pirate";
            ...
            elseif ((Level == 2425) or (Level <= 2449) or (SelectMonster == "Snow Demon")) then
                Ms = "Snow Demon";
                NameQuest = "CandyQuest1";
                QuestLv = 2;
                NameMon = "Snow Demon";
            ...
            elseif ((Level == 2450) or (Level <= 2474) or (SelectMonster == "Isle Outlaw")) then
                Ms = "Isle Outlaw";
            ...
            elseif ((Level == 2475) or (Level <= 2499) or (SelectMonster == "Island Boy")) then
                Ms = "Island Boy";
                NameQuest = "TikiQuest1";
                QuestLv = 2;
                NameMon = "Island Boy";
            ...
            elseif ((Level == 2500) or (Level <= 2524) or (SelectMonster == "Sun-kissed Warrior")) then
                ...
                CFrameMon = CFrame.new(- 16357.3125, 20.632822036743164, 1005.64892578125);
            elseif ((Level == 2525) or (Level <= 2549) or (SelectMonster == "Isle Champion")) then
                Ms = "Isle Champion";
                NameQuest = "TikiQuest2";
                QuestLv = 2;
            ...
            elseif ((Level == 2550) or (Level <= 2574) or (SelectMonster == "Serpent Hunter")) then
                Ms = "Serpent Hunter";
            ...
            elseif ((Level == 2575) or (Level <= 2599) or (SelectMonster == "Skull Slayer") or (Level == 2600)) then
                Ms = "Skull Slayer";
                NameQuest = "TikiQuest3";
                QuestLv = 2;
    ...
    v23.Draggable = true;
    v23.Image = "http://www.roblox.com/asset/?id=78774998900413";
    v24.Parent = v23;
    ...
    Tabs.Home:AddParagraph({
        Title = "Made by BinbinvoGaming",
        Content = "BinbinvoGaming Made This Script"
    })
    Tabs.Home:AddButton({
        Title = "Copy Link Discord",
        Description = "Copy Shinichi Hub Discord",
        Callback = function()
            setclipboard("https://discord.gg/89tsRNcC")
        end
    })
    -- function premium only
    ...
    local AutoCursedDualKatana = Tabs.PremiumOnly:AddToggle("ToogleAutoCursedDualKatana", {
        Title = "Auto Cursed Dual Katana", 
        Description = "Automatic Get Cursed Dual Katana ( CDK )",
        Default = false 
    })
    AutoCursedDualKatana:OnChanged(function(Value)
    ...
    local AutoLeviathan = Tabs.PremiumOnly:AddToggle("ToogleAutoLeviathan", {
        Title = "Auto Leviathan", 
        Description = "Automatic Leviathan ( BETA )",
        Default = false 
    })
    AutoLeviathan:OnChanged(function(Value)
        getgenv().KillLevi = Value
        SaveSettingsSoon()
    end)
    spawn(function()
        while task.wait(0.5) do
            if getgenv().KillLevi and Sea3 then
                pcall(function()
    ...
    local AutoSeaBeat = Tabs.PremiumOnly:AddToggle("ToogleAutoSeaBeat", {
        Title = "Auto Sea Beat", 
        Description = "Automatic Kill Sea Beat",
        Default = false 
    })
    AutoSeaBeat:OnChanged(function(Value)
        _G.AutoSeaBeat = Value
        SaveSettingsSoon()
    end)
    spawn(function()
    while wait() do
    ...
    local SelectMelee = Tabs.Main:AddDropdown("DropdownSelectWeapon", {
        Title = "Select Melee",
        Description = "Select your melee to farm with your melee",
        Values = {
            "Melee",
            "Sword",
            "Blox Fruits"
    ...
    SelectMelee:OnChanged(function(v236)
        ChooseWeapon = v236;
        SaveSettingsSoon()
    end);
    task.spawn(function()
    ...
    local v49 = Tabs.Main:AddToggle("ToggleLevel", {
        Title = "Auto Farm Level",
        Description = "Automatic Farm Level",
        Default = false
    });
    ...
    local v50 = Tabs.Main:AddToggle("ToggleMobAura", {
        Title = "Auto Farm Near Mob",
        Description = "Automatic Farm With Near Mob",
        Default = false
    });
    ...
    local v51 = Tabs.Main:AddToggle("ToggleCastleRaid", {
        Title = "Auto Pirates",
        Description = "Auto Pirates",
        Default = false
    });
    ...
    local v52 = Tabs.Main:AddToggle("ToggleHakiFortress", {
        Title = "Turn on Haki Color Fortress",
        Description = "Turn on Haki Color Fortress",
        Default = false
    });
    ...
    local v55 = Tabs.Main:AddToggle("ToggleCollectChest", {
        ...
        Description = "Automatic Collect Chest",
        Default = false
    });
    ...
    local v57 = Tabs.Main:AddDropdown("DropdownMastery", {
        Title = "Select Type Mastery",
        Description = "Select Type Mastery",
        Values = {
            "Near Mobs"
        },
        Multi = false,
    ...
    local v58 = Tabs.Main:AddToggle("ToggleMasteryFruit", {
        Title = "Auto Farm Mastery Fruits",
        Description = "Automatic Farm Mastery Fruits",
        Default = false
    });
    ...
    local v59 = Tabs.Main:AddSlider("SliderHealt", {
        Title = "Monster Blood",
        Description = "Select Monster Blood",
        Default = 20,
        Min = 0,
        Max = 100,
        Rounding = 1,
    ...
    if Sea3 then
        ...
        local v486 = Tabs.Main:AddToggle("ToggleBone", {
            ...
            Description = "Automatic Farm Bone",
            Default = false
        });
        ...
        local v489 = Tabs.Main:AddToggle("ToggleRandomBone", {
            ...
            Description = "Automatic Random Bone",
            Default = false
        });
    ...
    if Sea3 then
            ...
            while wait() do
                pcall(function()
                    if (string.len(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CakePrinceSpawner")) == 88) then
                        v491:SetDesc("Still: " .. string.sub(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CakePrinceSpawner"), 39, 41) .. "");
                    elseif (string.len(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CakePrinceSpawner")) == 87) then
                        v491:SetDesc("Still: " .. string.sub(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CakePrinceSpawner"), 39, 40) .. "");
                    elseif (string.len(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CakePrinceSpawner")) == 86) then
                        v491:SetDesc("Still: " .. string.sub(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CakePrinceSpawner"), 39, 39) .. " ");
                    else
        ...
        local v492 = Tabs.Main:AddToggle("ToggleCake", {
            ...
            Description = "Automaitc Farm Katakuri",
            Default = false
        });
        local v493 = true;
        ...
        local v494 = Tabs.Main:AddToggle("ToggleDoughKing", {
            Title = "Auto Kill Dough",
            Description = "Automatic Kill Dough",
            Default = false
        });
        ...
        local v495 = Tabs.Main:AddToggle("ToggleSpawnCake", {
            ...
            Description = "Automatic Spawn Katakuri",
            Default = true
        });
    ...
    if Sea2 then
        ...
        local v497 = Tabs.Main:AddToggle("ToggleVatChatKiDi", {
            ...
            Description = "Automatic Farm Ectoplasm",
            Default = false
        });
    ...
    local v61 = Tabs.Main:AddDropdown("DropdownBoss", {
        Title = "Select Boss",
        Description = "Select Boss To Farm Boss!",
        Values = tableBoss,
        Multi = false,
        Default = 1
    });
    ...
    local v62 = Tabs.Main:AddToggle("ToggleAutoFarmBoss", {
        Title = "Auto Kill Boss",
        Description = "Automatic Kill Boss In Dropdown",
        Default = false
    });
    ...
    local v63 = Tabs.Main:AddSection("Material");
    if Sea1 then
        MaterialList = {
            "Scrap Metal",
            "Leather",
    ...
    local v64 = Tabs.Main:AddDropdown("DropdownMaterial", {
    ...
    local v65 = Tabs.Main:AddToggle("ToggleMaterial", {
        Title = "Auto Farm Material",
        Description = "Automatic Farm Material",
        Default = false
    });
    ...
    if Sea3 then
        local v499 = Tabs.Event:AddParagraph({
        ...
        local v500 = Tabs.Event:AddToggle("ToggleEspKitsune", {
            Title = "Esp Kitsune Island",
            Description = "Show Esp Kitsune Island",
            Default = false
        });
        ...
        local v501 = Tabs.Event:AddToggle("ToggleTPKitsune", {
        ...
        local v502 = Tabs.Event:AddToggle("ToggleCollectAzure", {
            Title = "Auto Collect Azure",
            Description = "Automatic Collect Azure",
            Default = false
        });
    ...
    Tabs.Event:AddButton({
        Title = "Change Blue Soul",
        Description = "Change Blue Soul",
        Callback = function()
            game:GetService("ReplicatedStorage"):WaitForChild("Modules"):WaitForChild("Net"):WaitForChild("RF/KitsuneStatuePray"):InvokeServer();
        end
    });
    if Sea3 then
        local v503 = Tabs.Event:AddSection("Biển");
        ...
        local v508 = 350;
        local v509 = Tabs.Event:AddSlider("SliderSpeedBoat", {
            Title = "Speed Boat",
            Description = "Select The Speed For Boat",
            Default = v508,
            Min = 0,
            Max = 350,
            Rounding = 1,
        ...
        v509:SetValue(v508);
        local v510 = Tabs.Event:AddToggle("AutoFindPrehistoric", {
            Title = "Auto Find Prehistoric",
            Description = "Automatic Find Prehistoric",
            Default = false
        });
        ...
        v505.RenderStepped:Connect(function()
            ...
            if v594 then
                ...
                if not v513 then
                    Fluent:Notify({
                        Title = "Shinichi Hub",
                        Content = "Island Prehistoric Finded!",
                        Duration = 10
                    });
                    v513 = true;
                end
        ...
        local v514 = Tabs.Event:AddToggle("AutoFindMirage", {
            Title = "Auto Find Mirage",
            Description = "Automatic Find Mirage",
            Default = false
        });
        ...
        local v515 = Tabs.Event:AddToggle("AutoFindFrozen", {
            Title = "Auto Find Leviathan Island",
            Description = "Automatic Find Leviathan Island",
            Default = false
        });
        ...
        v505.RenderStepped:Connect(function()
            ...
            if v616 then
                ...
                if not v513 then
                    Fluent:Notify({
                        Title = "Shinichi HUb",
                        Content = "Island Leviathan Finded!",
                        Duration = 10
                    });
                    v513 = true;
                end
        ...
        local v516 = Tabs.Event:AddToggle("AutoComeTiki", {
            Title = "Auto Sailing to Tiki Island",
            Description = "Automatic Sailing to Tiki Island",
            Default = false
        });
        ...
        local v517 = Tabs.Event:AddToggle("AutoComeHydra", {
            Title = "Auto Sailing to Hydra Island",
            Description = "Automatic Sailing to Hydra Island",
            Default = false
        });
        ...
        Tabs.Event:AddButton({
            ...
            Description = "Fly To Hunting Grounds",
            Callback = function()
        ...
        local v519 = Tabs.Event:AddDropdown("DropdownBoat", {
            Title = "Select Boat",
            Description = "Select The Boat",
            Values = v518,
            Multi = false,
            Default = 1
        });
        ...
        local v522 = Tabs.Event:AddToggle("ToggleTerrorshark", {
            Title = "Auto Shark",
            Description = "Automatic Shark",
            Default = false
        });
        ...
        local v523 = Tabs.Event:AddToggle("TogglePiranha", {
            Title = "Auto Piranha",
            Description = "Automatic Piranha",
            Default = false
        });
        ...
        local v524 = Tabs.Event:AddToggle("ToggleShark", {
            Title = "Auto Fry",
            Description = "Automatic Fry",
            Default = false
        });
        ...
        local v525 = Tabs.Event:AddToggle("ToggleFishCrew", {
            Title = "Auto Fish Crew",
            Description = "Automatic Fish Crew",
            Default = false
        });
        ...
        local v526 = Tabs.Event:AddToggle("ToggleShip", {
            Title = "Auto Ship",
            Description = "Automatic Ship",
            Default = false
        });
        ...
        local v527 = Tabs.Event:AddToggle("ToggleGhostShip", {
            Title = "Auto Ghost Ship",
            Description = "Automatic Ghost Ship",
            Default = false
        });
    ...
    if Sea3 then
        local v532 = Tabs.Event:AddParagraph({
            Title = "Status Mirage",
            Content = ""
        });
        task.spawn(function()
        ...
        spawn(function()
            pcall(function()
                while wait() do
                    v532:SetDesc("Mirage Islands: " .. MirageStatus .. " | Full Moon: " .. FullMoonStatus);
                end
            end);
        end);
        Tabs.Event:AddButton({
            Title = "Fly To High Places",
            Description = "Fly To High Places",
            Callback = function()
                TweenToHighestPoint();
            end
        });
    ...
    local v66 = Tabs.Event:AddToggle("ToggleTpAdvanced", {
        Title = "Fly To Advanced Fruit Dealer",
        Description = "Fly To Advanced Fruit Dealer",
        Default = false
    });
    v66:OnChanged(function(v257)
    
        SaveSettingsSoon()end);
    spawn(function()
        while wait() do
    ...
    local v67 = Tabs.Event:AddToggle("ToggleTweenGear", {
    ...
    local v68 = Tabs.Event:AddToggle("Togglelockmoon", {
        Title = "Look at the Moon and Use the Clan",
        Description = "Look at the Moon and Use the Clan",
        Default = false
    });
    ...
    if Sea3 then
        ...
        local v72 = Tabs.Item:AddToggle("ToggleAutoSaw", {
            ...
            Description = "Automatic Shark Saw",
            Default = false
        });
    ...
    local v75 = Tabs.Item:AddToggle("ToggleAutoCanvander", {
        ...
        Description = "Automatic Cavender",
        Default = false
    });
    ...
    local v76 = Tabs.Item:AddToggle("ToggleAutoMusketeerHat", {
        ...
        Description = "Automatic Musketeer Hat",
        Default = false
    });
    ...
    local v77 = Tabs.Item:AddToggle("ToggleAutoObservationV2", {
        Title = "Auto Haki Observe V2",
        Description = "Automatic Haki Observe V2",
        Default = false
    });
    ...
    local v78 = Tabs.Item:AddToggle("ToggleAutoRainbowHaki", {
        Title = "Auto Haki Rainbow Color",
        Description = "Automatic Haki Rainbow Color",
        Default = false
    });
    ...
    local v79 = Tabs.Item:AddToggle("ToggleAutoSkullGuitar", {
        ...
        Description = "Automatic Skull Guitar",
        Default = false
    });
    ...
    local v82 = Tabs.Item:AddToggle("ToggleAutoDualKatana", {
        ...
        Description = "Automatic Dual Katana",
        Default = false
    });
    ...
    if Sea2 then
        local v537 = Tabs.Item:AddToggle("ToggleFactory", {
            Title = "Auto Factory",
            Description = "Automatic Factory",
            Default = false
        });
    ...
    local v83 = Tabs.Item:AddToggle("ToggleAutoFarmSwan", {
        Title = "Auto Farm Swan",
        Description = "Automatic Farm Swan",
        Default = false
    });
    ...
    if (Sea2 or Sea3) then
        local v538 = Tabs.Item:AddToggle("ToggleHakiColor", {
            Title = "Auto Buy Color Haki",
            Description = "Automatic Buy Color Haki",
            Default = false
        });
    ...
    if Sea2 then
        local v539 = Tabs.Main:AddToggle("ToggleSwordLengend", {
            ...
            Description = "Automatic Buy Sword Lengend",
            Default = false
        });
    ...
    local v85 = Tabs.Setting:AddToggle("ToggleAutoT", {
        Title = "Auto On Clan V3",
        Description = "Automatic On Clan V3",
        Default = false
    });
    ...
    local v86 = Tabs.Setting:AddToggle("ToggleAutoY", {
        Title = "Auto On Clan V4",
        Description = "Automatic On Clan V4",
        Default = false
    });
    ...
    local v87 = Tabs.Setting:AddToggle("ToggleAutoKen", {
        Title = "Auto On Observation Haki",
        Description = "Automatic On Observation Haki",
        Default = false
    });
    ...
    local v88 = Tabs.Setting:AddToggle("ToggleSaveSpawn", {
        ...
        Description = "Automatic Save Spawn",
        Default = false
    });
    ...
    local v90 = Tabs.Setting:AddToggle("ToggleBringMob", {
        Title = "Bring Mob",
        Description = "Bring Mob To Farm Fast",
        Default = true
    });
    ...
    local v91 = Tabs.Setting:AddToggle("ToggleRemoveNotify", {
        ...
        Description = "Remove Notificaton In Blox Frutis",
        Default = false
    });
    ...
    local v92 = Tabs.Setting:AddToggle("ToggleWhite", {
        Title = "White Screen",
        Description = "Turn On To Fix Lag",
        Default = false
    });
    ...
    local function v103()
        ...
        local v300 = v297 % 60 ;
        v102:SetDesc(string.format("%02d Hour-%02d Minutes-%02d Second", v298, v299, v300));
    end
    spawn(function()
        while task.wait() do
            pcall(v103);
    ...
    local v107 = Tabs.Stats:AddToggle("ToggleMelee", {
        ...
        Description = "Automatic Stats Melee",
        Default = false
    });
    ...
    local v108 = Tabs.Stats:AddToggle("ToggleDe", {
        ...
        Description = "Automatic Stats Defense",
        Default = false
    });
    ...
    local v109 = Tabs.Stats:AddToggle("ToggleSword", {
        ...
        Description = "Automatic Stats Sword",
        Default = false
    });
    ...
    local v110 = Tabs.Stats:AddToggle("ToggleGun", {
        ...
        Description = "Automatic Stats Gun",
        Default = false
    });
    ...
    local v111 = Tabs.Stats:AddToggle("ToggleFruit", {
        Title = "Auto Stats Frutis",
        Description = "Automatic Stats Fruits",
        Default = false
    });
    ...
    local v113 = Tabs.Player:AddDropdown("SelectedPly", {
        Title = "Select Player",
        Description = "Select Player In Server",
        Values = v112,
        Multi = false,
        Default = 1
    });
    ...
    local v56 = Tabs.Player:AddSection("Khác");
    ...
    local v130 = Tabs.Visual:AddToggle("ToggleEspPlayer", {
        Title = "Esp Player",
    ...
    local v131 = Tabs.Visual:AddToggle("ToggleEspFruit", {
        Title = "Esp Fruits",
        Description = "Esp Fruits",
        Default = false
    });
    ...
    local v132 = Tabs.Visual:AddToggle("ToggleEspIsland", {
        Title = "Esp Islands",
        Description = "Esp Islands",
        Default = false
    });
    ...
    local v133 = Tabs.Visual:AddToggle("ToggleEspFlower", {
        Title = "Esp Flower",
    ...
    local v134 = Tabs.Visual:AddToggle("ToggleEspRealFruit", {
        Title = "Pineapple Apple",
        Description = "Pineapple Apple",
        Default = false
    });
    ...
    local v135 = Tabs.Visual:AddToggle("ToggleIslandMirageEsp", {
    ...
    local v123 = Tabs.Fruit:AddDropdown("DropdownFruit", {
        Title = "Select Fruits",
        Description = "Select Fruits To Buy",
    ...
    local v111 = Tabs.Fruit:AddToggle("ToggleFruit", {
        ...
        Description = "Automatic Buy Fruit",
        Default = false
    });
    ...
    local v124 = Tabs.Fruit:AddDropdown("DropdownPermanentFruit", {
        ...
        Description = "Select Permanent Frutis",
        Values = Table_DevilFruitSniper,
        Multi = false,
        Default = 1
    });
    ...
    local v125 = Tabs.Fruit:AddToggle("TogglePermanentFruit", {
        Title = "Switch Fruits Permanent",
        Description = "Switch Frutis Permanent",
        Default = false
    });
    ...
    local v126 = Tabs.Fruit:AddToggle("ToggleStore", {
        ...
        Description = "Automatic Store Frutis",
        Default = false
    });
    ...
    local v128 = Tabs.Fruit:AddToggle("ToggleCollectTP", {
        Title = "Teleport To Fruits 1",
        Description = "Teleport To Fruits",
        Default = false
    });
    ...
    local v138 = Tabs.Raid:AddToggle("ToggleBuy", {
        Title = "Auto Buy Chip",
        Description = "Automatic Buy Chip",
        Default = false
    });
    ...
    local v139 = Tabs.Raid:AddToggle("ToggleStart", {
        ...
        Description = "Automatic Start Raid",
        Default = false
    });
    ...
    local v140 = Tabs.Raid:AddToggle("ToggleNextIsland", {
        Title = "Kill Monster And Teleport To Next Island",
        Description = "Kill Monster And Teleport To Next Island",
        Default = false
    });
    ...
    local v142 = Tabs.Raid:AddToggle("ToggleGetFruit", {
        Title = "Get fruit under 1 million beli",
        Description = "Get fruit under 1 million beli",
        Default = false
    });
    ...
    if Sea2 then
        Tabs.Raid:AddButton({
            Title = "Teleport To Raid Islands",
            Description = "Teleport To Raid Islands",
            Callback = function()
                Tween2(CFrame.new(- 6438.73535, 250.645355, - 4501.50684));
            end
        });
    ...
    local v143 = Tabs.Raid:AddToggle("ToggleLaw", {
        Title = "Buy Chip And Kill Law",
        Description = "Buy Chip And Kill Law",
        Default = false
    });
    ...
    Tabs.Race:AddButton({
        Title = "Place Buy Gear",
        Description = "Place Buy Gear",
        Callback = function()
            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(28286.35546875, 14895.3017578125, 102.62469482421875));
    ...
    local v144 = Tabs.Race:AddToggle("ToggleHumanandghoul", {
        ...
        Description = "Complete Stage [Human/Ghoul]",
        Default = false
    });
    ...
    local v146 = Tabs.Race:AddToggle("ToggleKillTrial", {
        Title = "Kill Player In Trial",
        Description = "Kill Player In Trial",
        Default = false
    });
    ...
    local v56 = Tabs.Race:AddSection("Huấn Luyện");
    local v147 = Tabs.Race:AddToggle("ToggleFarmRace", {
        Title = "Farm Race",
        Description = "Farm Race",
        Default = false
    });
    local v148 = false;
    ...
    local v56 = Tabs.Shop:AddSection("Melee");
    Tabs.Shop:AddButton({
        Title = "Black Leg",
    ...
    Tabs.Shop:AddButton({
        ...
        Description = "Change Race Cyborg",
        Callback = function()
            local v367 = {
                [1] = "CyborgTrainer",
                [2] = "Buy"
    ...
    Tabs.Misc:AddButton({
        ...
        Description = "Automatic Awakening",
        Callback = function()
    ...
    local v56 = Tabs.Event:AddSection("Leviathan");
    Tabs.Event:AddButton({
        Title = "Buy Chip Leviathan",
        Description = "Buy Chip Leviathan",
        Callback = function()
            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("InfoLeviathan", "2");
        end
    });
    local v155 = Tabs.Event:AddToggle("ToggleTPFrozenDimension", {
        Title = "Teleport To Island Leviathan",
        Description = "Teleport To Leviathan Island",
        Default = false
    });
    ...
    if Sea3 then
        local v561 = Tabs.Event:AddParagraph({
    ...
    local v156 = Tabs.Event:AddSection("Draco");
    local v157 = Tabs.Event:AddToggle("ToggleBlazeEmber", {
        Title = "Get Red Fire",
        Description = "Get Red Fire",
        Default = false
    });
    ...
    local v158 = Tabs.Event:AddToggle("ToggleReceiveQuest", {
        Title = "Get Quest Red Fire",
        Description = "Get Quest Red Fire",
        Default = false
    });
    ...
    local v159 = Tabs.Event:AddParagraph({
        Title = "Status Quest Red Fire",
        Content = ""
    });
    spawn(function()
        pcall(function()
    ...
    local v160 = Tabs.Event:AddToggle("ToggleHydraTree", {
        Title = "Tree Destruction on Hydra Island",
        Description = "Tree Destruction on Hydra Island",
        Default = false
    });
    ...
    v156:AddButton({
        ...
        Description = "Teleport To Dragon Dojo",
        Callback = function()
            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(5661.5322265625, 1013.0907592773438, - 334.9649963378906));
            Tween2(CFrame.new(5814.42724609375, 1208.3267822265625, 884.5785522460938));
        end
    ...
    v156:AddButton({
        ...
        Description = "Create Volcanic Magnet",
        Callback = function()
            local v395 = {
                [1] = "CraftItem",
                [2] = "Craft",
    ...
    local v164 = Tabs.Event:AddToggle("ToggleCollectFireFlowers", {
        Title = "Get Red Flower",
        Description = "Get Red Flower",
        Default = false
    });
    ...
    local v165 = Tabs.Event:AddToggle("ToggleWhiteBelt", {
        Title = "Farm White Belt",
        Description = "Farm White Belt",
        Default = false
    });
    ...
    local v167 = Tabs.Event:AddToggle("ToggleTrialTeleport", {
        Title = "Teleport To Door Trial Race Draco",
        Description = "Teleport To Door Trial Race Draco",
        Default = false
    });
    ...
    local v169 = Tabs.Event:AddParagraph({
    ...
    local v170 = Tabs.Event:AddToggle("ToggleTPVolcano", {
    ...
    local v171 = Tabs.Event:AddToggle("ToggleDefendVolcano", {
        Title = "Defense",
        Description = "Defense",
        Default = false
    });
    ...
    local v107 = Tabs.Event:AddToggle("ToggleMelee", {
        Title = "Using Melee",
        Description = "Using Melee",
        Default = false
    });
    ...
    local v109 = Tabs.Event:AddToggle("ToggleSword", {
        Title = "Using Sword",
        Description = "Using Sword",
        Default = false
    });
    ...
    local v110 = Tabs.Event:AddToggle("ToggleGun", {
        Title = "Using Gun",
        Description = "Using Gun",
        Default = false
    });
    ...
    local v175 = Tabs.Event:AddToggle("ToggleKillAura", {
        Title = "Auto Kill Aura",
        Description = "Automatic kill aura",
        Default = false
    });
    ...
    local v176 = Tabs.Main:AddToggle("ToggleCollectBone", {
        Title = "Auto Farm Bone",
        Description = "Automatically Plow Bones Until Turned Off",
        Default = false
    });
    ...
    local v177 = Tabs.Event:AddToggle("ToggleCollectEgg", {
        Title = "Auto Collect Egg",
        Description = "Automatic collect egg",
        Default = false
    });
